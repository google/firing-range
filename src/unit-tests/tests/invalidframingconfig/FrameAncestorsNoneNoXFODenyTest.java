...
/** Tests for {@link FrameAncestorsNoneNoXFODeny}. */
@RunWith(JUnit4.class)
public final class FrameAncestorsNoneNoXFODenyTest {
  private final HttpServletRequest request = mock(HttpServletRequest.class);
  private final HttpServletResponse response = mock(HttpServletResponse.class);
  private final PrintWriter writer = mock(PrintWriter.class);

  @Test
  public void doGet_always_setsCSPOptionButNoXFODeny() throws IOException {
    when(response.getWriter()).thenReturn(writer);
    new FrameAncestorsNoneNoXFODeny().doGet(request, response);
